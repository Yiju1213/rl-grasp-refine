...
FIGURE_STEM = "cn_fig10_full_training_curves"
CM_TO_INCH = 1.0 / 2.54
FIGURE_WIDTH_CM = 12.0
FIGURE_HEIGHT_CM = FIGURE_WIDTH_CM * 9.0 / 16.0
REPO_ROOT = PLOT_SCRIPTS_DIR.parent
...
METRIC_DISPLAY_NAMES_CN = {
    "validation/outcome/success_lift_vs_dataset": "验证集",
    "outcome/success_lift_vs_dataset": "训练集",
}
METRIC_STYLES_CN = {
    **METRIC_STYLES,
...
def plot_training_curves_cn(plot_frame, *, dpi: int, out_dir: Path | str) -> list[Path]:
    ...
    for metric, metric_frame in plot_frame.groupby("metric", sort=False):
        ...
        mean = ordered["mean"].to_numpy(dtype=float) * 100.0
        std = ordered["std"].to_numpy(dtype=float) * 100.0
    ...
    ax.set_ylabel("相对数据集抓取结果的成功率增益（%）", fontproperties=CN_FONT)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="中文绘制 Full policy 三个 seed 的训练曲线。")
    parser.add_argument(
        "--run-dirs",
        nargs="+",
